...
from misc.utils import gumbel_sampling, to_one_hot, special_cutmix
from misc.loss import RankMixup_MRL, RankMixup_MNDCG
...
def train_epoch(args, epoch, model, train_loader, optimizer, criterion):
    ...
    for step, (data, label, filename) in enumerate(train_loader):
        ...
        # length : 3
        synthetic_samples, synthetic_labels, lams = special_cutmix(data, data_j, perm_label, perm_label_j)
        
        new_data = torch.cat(synthetic_samples, dim=0) # [N x 3, 3, 224, 224]
        new_label = torch.cat(synthetic_labels, dim=0) # [N x 3, 7]
        
        new_output = model(new_data) # [N x 3, 7]
        
        loss, loss_ce, loss_mixup = rank_criterion(output, label, new_output, new_label, lams)
